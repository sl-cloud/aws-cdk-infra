...
def test_secrets_stack_creation(app, dev_config):
    ...
    template.resource_count_is("AWS::KMS::Key", 2)  # Secrets KMS key + SSM outputs KMS key
...
def test_ssm_parameters_created(app, dev_config):
    ...
    template.resource_count_is("AWS::SSM::Parameter", 9)  # 4 ARNs + 4 names + 1 KMS key ARN
